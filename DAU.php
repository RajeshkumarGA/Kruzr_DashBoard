<?php
	...
	if($server)
	{
		$sql_query_user_details = "SELECT * FROM $database.userDetails WHERE createdDate  between '$current_date' and '$next_date' and iduserDetails IN(SELECT DISTINCT uid FROM $database.TripDetails)";
	...
	{
		$sql_query_user_details = "SELECT * FROM userdetails WHERE createdDate  between '$current_date' and '$next_date' and iduserDetails IN(SELECT DISTINCT uid FROM tripdetails)";
